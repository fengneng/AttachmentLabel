...
NSString *const MyBaseAttachmentAttributeName = @"MyBaseAttachment";

@interface MyBaseTextLayout () {
@protected
    CTFrameRef _ctFrame;
    CGRect _rect;
    NSAttributedString *_attributedString;
    NSArray<__kindof MyBaseAttachment *> *_attachments;
}

@end

@implementation MyBaseTextLayout
...
- (void)fetchAttachment {
    ...
    for (int i = dataIndex; i < lineCount; ++i) {
        ...
        for (id runObj in runObjArray) {
            ...
            MyBaseAttachment *attachment = runAttributes[MyBaseAttachmentAttributeName];
            if (!attachment) continue;

            CTRunDelegateRef delegate = (__bridge CTRunDelegateRef)[runAttributes valueForKey:(id)kCTRunDelegateAttributeName];

            MyBaseTextRunDelegate * runDelegate = CTRunDelegateGetRefCon(delegate);
            if (![runDelegate isKindOfClass:[MyBaseTextRunDelegate class]]) continue;
...
@implementation MyBaseTextLayout (Draw)
...
- (void)drawRect:(CGRect)rect {
    ...
    for (MyBaseAttachment *attachment in _attachments) {
...
@implementation MyBaseTextRunDelegate

static void deallocCallback(void *ref) {
    MyBaseTextRunDelegate *self = (__bridge_transfer MyBaseTextRunDelegate *)(ref);
    self = nil; // release
}

static CGFloat ascentCallback(void *ref){
    MyBaseTextRunDelegate *delegate = (__bridge MyBaseTextRunDelegate *)(ref);
